...
internal class NostrViewer
{
    ...
    public void Subscribe()
    {
        ...
        events
            ...
            .Subscribe(x =>
            {
                ...
                if (x.Tags?.Any() ?? false)
                {
                    foreach (var tag in x.Tags.Where(t => !string.IsNullOrEmpty(t.TagIdentifier) && t.TagIdentifier.Equals("p", StringComparison.OrdinalIgnoreCase) && t.AdditionalData != null))
                    {
                        var followData = tag.AdditionalData.ToList();
                        var followsHex = followData[0];
                        var suggestedRelay = followData.Count > 1 ? followData[1] : string.Empty;
                        var petName = followData.Count > 2 ? followData[2] : string.Empty;
                        Log.Information("Follows: {followsHex}, Relay: {suggestedRelay}, PetName: {petName}", followsHex, suggestedRelay, petName);
                    }
                }
            });
    ...
    {
        var privKey = NostrPrivateKey.FromBech32("nsec18c8f4gvjq0rwex3amyym797k6j9l827s7c5czpekxs5rn2nydz5q8v5g34");
        _client.SendProfileRequest(privKey, NostrKind.Metadata, NostrKind.Contacts);
